...
main()`,
    steps: [
      {
        ...
        desc: 'An exception isn\'t a crash. It\'s an <strong>object</strong> that travels up the call stack, clearing one desk at a time, until some frame says it will deal with it. In this demo no frame does, so you get to see the whole journey.',
        lines: [],
        memory: EMPTY,
      },
      {
        title: 'Three ordinary function objects',
        desc: 'Session 02, unchanged: each <code>def</code> builds a function object and binds a name. The <code>raise</code> on line 2 is just a line of code inside <code>load</code>. Nothing has been raised yet.',
        lines: [1, 4, 7],
        memory: {
          frames: [{ name: 'global', vars: [
      ...
      },
      {
        ...
        desc: 'Each call opens a frame on top of the last. Every frame below the top one is paused on the line that made the call, waiting for a value to come back. Keep that picture in mind. It\'s what the exception is about to walk through.',
        lines: [10, 8, 5],
        memory: {
          frames: [
            { name: 'global', vars: [
      ...
      },
      {
        ...
        desc: '<code>parse</code> had no <code>try</code> around line 5, so its frame goes too. Now <code>main</code> is asked, at line 8. Three entries in the traceback, three desks walked through. Nothing in your code has been consulted yet. This is bookkeeping the interpreter does between lines.',
        lines: [5, 8],
        memory: {
          frames: [
            { name: 'global', vars: [
      ...
      {
        title: 'It reaches the module level, and there\'s no handler anywhere',
        ...
        memory: {
          ...
          heap: [
            exception('exc1', ADDRS.exc1, 'ValueError', ADDRS.ValueErrorCls, 'bad data', {
              ...
              note: 'four frames deep, and this list is what gets printed' }),
            { id: 'loadFn',  pyId: ADDRS.loadFn,  type: 'function', value: 'load()',  refcount: 1, mutable: false, state: 'normal' },
            { id: 'parseFn', pyId: ADDRS.parseFn, type: 'function', value: 'parse()', refcount: 1, mutable: false, state: 'normal' },
            { id: 'mainFn',  pyId: ADDRS.mainFn,  type: 'function', value: 'main()',  refcount: 1, mutable: false, state: 'normal' },
          ],
        ...
        memory: {
          ...
          heap: [
            exception('exc1', ADDRS.exc1, 'ValueError', ADDRS.ValueErrorCls, 'bad data', {
              ...
              note: 'printed to stderr, then the whole process, this object included, is gone' }),
            { id: 'loadFn',  pyId: ADDRS.loadFn,  type: 'function', value: 'load()',  refcount: 1, mutable: false, state: 'normal' },
            { id: 'parseFn', pyId: ADDRS.parseFn, type: 'function', value: 'parse()', refcount: 1, mutable: false, state: 'normal' },
            { id: 'mainFn',  pyId: ADDRS.mainFn,  type: 'function', value: 'main()',  refcount: 1, mutable: false, state: 'normal' },
          ],
  ...
  /* ──────────────────────────────────────────────────────────
    ...
    watch: 'Watch <code>main</code>\'s desk. It survives, <code>err</code> appears on it, and then <code>err</code> is deleted <em>before</em> the function returns.',
    code: `def load():
    raise ValueError("bad data")
...
result = main()`,
    steps: [
      {
        title: 'A handler is a frame that says "I\'ll deal with that"',
        desc: 'Same <code>load</code>, same <code>raise</code>. The one difference is on lines 5 to 7: <code>main</code> wraps its call in a <code>try</code>. That\'s enough to stop the unwinding one desk up, and to show you what <code>as err</code> really binds.',
        lines: [],
        memory: EMPTY,
      },
      {
        title: 'Two functions, and <code>main</code> has a <code>try</code> around its call',
        desc: 'Defining a function with a <code>try</code> in it costs nothing at definition time. The <code>except</code> clause isn\'t registered anywhere. It\'s a region of <code>main</code>\'s code that the interpreter will look up <em>only if</em> an exception arrives while line 6 is executing.',
        lines: [1, 4, 5],
        memory: {
          frames: [{ name: 'global', vars: [
      ...
      {
        title: '<code>raise</code> builds the exception, and <code>load</code> has no handler',
        desc: 'Just as in demo 1: an instance appears, a traceback entry for <code>load</code> is attached, and <code>load</code>\'s desk is cleared because nothing in it can handle a <code>ValueError</code>.',
        lines: [2],
        memory: {
          frames: [
            { name: 'global', vars: [
        ...
        memory: {
          ...
          heap: [
            exception('exc2', ADDRS.exc2, 'ValueError', ADDRS.ValueErrorCls, 'bad data', {
              ...
              note: 'isinstance(exc, ValueError) is True: the search stops here' }),
            { id: 'loadFn2', pyId: ADDRS.loadFn2, type: 'function', value: 'load()', refcount: 1, mutable: false, state: 'normal' },
            { id: 'mainFn2', pyId: ADDRS.mainFn2, type: 'function', value: 'main()', refcount: 1, mutable: false, state: 'normal' },
          ],
          highlight: ['exc2'],
      ...
      },
      {
        ...
        desc: 'Nothing magical here. <code>err</code> is a name on <code>main</code>\'s desk pointing at the instance, at the same address as before. The handler body runs and prints <code>recovered: bad data</code>. While it runs, the interpreter also holds a reference of its own, so <code>sys.exception()</code> can hand the object to a bare <code>raise</code>.',
        lines: [7, 8],
        memory: {
          frames: [
            { name: 'global', vars: [
      ...
      },
      {
        ...
        desc: 'This catches everyone out once. At the end of an <code>except ... as err</code> block Python runs <code>del err</code> for you, so <code>print(err)</code> on line 9 would raise <code>NameError</code>. The reason is a cycle: the exception holds the traceback, the traceback holds the frames it passed through, and this frame would hold <code>err</code>, so nothing would ever be freed. With <code>err</code> gone and the handler finished, the exception\'s count hits zero and it takes the traceback with it. Bind it to another name if you need it later.',
        lines: [8, 9],
        memory: {
          frames: [
            { name: 'global', vars: [
          ...
          heap: [
            exception('exc2', ADDRS.exc2, 'ValueError', ADDRS.ValueErrorCls, 'bad data', {
              ...
              note: 'no name and no handler holds it, so it is freed, traceback and all' }),
            { id: 'loadFn2', pyId: ADDRS.loadFn2, type: 'function', value: 'load()', refcount: 1, mutable: false, state: 'normal' },
            { id: 'mainFn2', pyId: ADDRS.mainFn2, type: 'function', value: 'main()', refcount: 1, mutable: false, state: 'normal' },
          ],
          highlight: ['exc2'],
      ...
      },
      {
        ...
        desc: 'Line 9 runs, <code>main</code> returns <code>"fallback"</code>, and the global name <code>result</code> is bound to it. Nothing below the handler ever knew an exception existed. A caught exception is an object that was built, used for a few lines, and discarded. The stack only looks dramatic while it\'s in flight.',
        lines: [9, 11],
        memory: {
          frames: [{ name: 'global', vars: [
            { name: 'load',   ref: 'loadFn2',     pyId: ADDRS.loadFn2,     type: 'function', state: 'normal' },
  ...
  /* ──────────────────────────────────────────────────────────
    ...
    watch: 'The <code>return</code> on line 4 doesn\'t leave the frame. Watch <code>closed</code> flip to <code>True</code> first, and only then the desk go.',
    code: `def read(path):
    f = open(path)
    try:
        return f.read()
...
text = read("notes.txt")`,
    steps: [
      ...
      },
      {
        ...
        desc: '<code>open</code> returns an ordinary instance (Session 04) whose class is <code>TextIOWrapper</code>. Behind it sits something the diagram can\'t draw: a real operating-system file handle, a limited resource that Python doesn\'t get back until this object is closed.',
        lines: [8, 2],
        memory: {
          frames: [
            { name: 'global', vars: [
      ...
      },
      {
        ...
        desc: 'Here\'s the part people don\'t expect. <code>return</code> evaluates its expression, so the file is read and a string is built, but the frame is <strong>not</strong> left yet. Because the <code>return</code> sits inside a <code>try</code> that has a <code>finally</code>, the result is parked and control jumps to line 5.',
        lines: [4],
        memory: {
          frames: [
            { name: 'global', vars: [
        ...
        memory: {
          ...
          heap: [
            ...
            { id: 'fileObj', pyId: ADDRS.fileObj, type: 'instance', value: 'open("notes.txt")', refcount: 0, mutable: true, state: 'gc',
              ...
              note: 'the name f went with the frame. Already closed, so nothing leaks' },
          ],
          highlight: ['strText', 'fileObj'],
        },
      },
      {
        title: 'Summary: what <code>finally</code> guarantees, and what it doesn\'t',
        desc: 'It guarantees the block runs on every exit path. It does <em>not</em> cancel an exception in flight. The exception resumes afterwards, unless the <code>finally</code> block itself does a <code>return</code>, which quietly swallows it. That bug is common enough that Python 3.14 warns about <code>return</code> inside <code>finally</code>. Demo 4 shows the idiom that packages this whole shape into two lines: <code>with</code>.',
        lines: [3, 5],
        memory: {
          frames: [{ name: 'global', vars: [
            { name: 'read', ref: 'readFn',  pyId: ADDRS.readFn,  type: 'function', state: 'normal' },
  ...
  /* ──────────────────────────────────────────────────────────
     ...
     ────────────────────────────────────────────────────────── */
  with: {
    watch: 'Two method calls you never wrote: <code>__enter__</code> before the body and <code>__exit__</code> after it. Watch what <code>as r</code> is bound to.',
    code: `class Resource:
    def __enter__(self):
        print("open")
        return self
    ...
    steps: [
      {
        ...
        desc: 'A <strong>context manager</strong> is any object whose class defines <code>__enter__</code> and <code>__exit__</code>. The <code>with</code> statement is the code that calls them for you, in a fixed order, with a <code>try/finally</code> around the body, just like demo 3.',
        lines: [],
        memory: EMPTY,
      },
      {
        title: 'The class holds <code>__enter__</code> and <code>__exit__</code>',
        desc: 'Session 04\'s picture: a class object with two function objects in its namespace. Nothing has been entered. These names are only special because <code>with</code> knows to look them up. There\'s no registration, no decorator, and no base class to inherit from.',
        lines: [1, 2, 6],
        memory: {
          frames: [{ name: 'global', vars: [
      ...
      },
      {
        ...
        desc: 'It returned <code>self</code>, so <code>r</code> is the same instance, at the same address. It didn\'t have to. <code>open()</code> returns the file, <code>threading.Lock().__enter__</code> returns <code>True</code>, and plenty of managers return <code>None</code>, which is why <code>with lock:</code> without <code>as</code> is so common. The name lands on the <em>enclosing</em> desk, not on a desk of its own.',
        lines: [4, 10],
        memory: {
          frames: [{ name: 'global', vars: [
            { name: 'Resource', ref: 'ResourceCls', pyId: ADDRS.ResourceCls, type: 'class', state: 'normal' },
      ...
      },
      {
        ...
        desc: 'Three <code>None</code>s mean "the body finished without an exception". This call is the <code>finally</code> block of demo 3, given a name and moved into the class, so every user of <code>Resource</code> gets the cleanup without writing it. Its return value, <code>False</code>, is ignored on the success path.',
        lines: [6, 7, 8],
        memory: {
          frames: [
            { name: 'global', vars: [
    ...
    steps: [
      {
        ...
        desc: 'This case is the whole reason context managers exist. The body raises, the resource still has to be released, and then the exception has to keep going so that somebody up the stack finds out. Watch all three happen in order.',
        lines: [],
        memory: EMPTY,
      },
      {
        title: 'Enter, just as in demo 4',
        desc: 'Instance built, <code>__enter__</code> called, <code>r</code> bound to what it returned. The with statement holds its own reference to the instance so it can call <code>__exit__</code> later, even if the body rebinds <code>r</code> to something else.',
        lines: [9, 2, 3],
        memory: {
          frames: [{ name: 'global', vars: [
      ...
      },
      {
        ...
        desc: 'Demo 1\'s first step, at module level: an instance of <code>ValueError</code>, with a traceback entry for the frame it was raised in. The line after the body, <code>print("never printed")</code>, is now unreachable. The only question is whether the exception leaves this frame.',
        lines: [10],
        memory: {
      ...
      },
      {
        ...
        desc: 'The with statement catches the exception before it can go anywhere and calls <code>__exit__(type(exc), exc, exc.__traceback__)</code>. Compare the desk with demo 4: <code>exc_type</code> points at the class, <code>exc</code> at the instance, <code>tb</code> at its traceback. The cleanup runs, <code>close</code> is printed, and it knows what went wrong.',
        lines: [5, 6],
        memory: {
          frames: [
            { name: 'global', vars: [
      ...
      {
        title: '<code>return False</code> means "I didn\'t handle it", so the exception carries on',
        desc: 'The with statement re-raises the same object, as if the <code>with</code> had never been there, except that the cleanup has happened. A bare function returns <code>None</code>, which counts as <code>False</code>, so this is the default. Returning <code>True</code> would swallow the exception and line 12 would print. <code>contextlib.suppress</code> is an <code>__exit__</code> that returns <code>True</code> for the classes you name.',
        lines: [7, 10],
        memory: {
          ...
          heap: [
            exception('exc5', ADDRS.exc5, 'ValueError', ADDRS.ValueErrorCls5, 'boom', {
              state: 'mutated', tb: '<module> (line 10)', note: 'the same object continues its journey, and there is no handler at module level' }),
            { id: 'ResourceCls5', pyId: ADDRS.ResourceCls5, type: 'class', value: 'class Resource', refcount: 2, mutable: true, state: 'normal', pairs: [
              { key: '__enter__', value: 'fn -> 0x7fc500c0', type: 'ref' },
              { key: '__exit__',  value: 'fn -> 0x7fc50140', type: 'ref' },
            ] },
    ...
    steps: [
      {
        ...
        desc: 'Real handlers often can\'t fix the problem, so they translate it. A low-level <code>ValueError</code> from <code>int()</code> becomes a <code>RuntimeError</code> that means something to the caller. Python keeps both objects, links them, and prints both, as long as you tell it how they\'re related.',
        lines: [],
        memory: EMPTY,
      },
      {
        title: '<code>parse("x1")</code>: <code>int()</code> raises inside the <code>try</code>',
        desc: '<code>int</code> is written in C, so there\'s no Python frame for it. The first desk the exception lands on is <code>parse</code>\'s, at line 3. Its message is the one you\'ve seen a hundred times: <code>invalid literal for int() with base 10: \'x1\'</code>.',
        lines: [7, 3],
        memory: {
          frames: [
            { name: 'global', vars: [
      ...
      },
      {
        ...
        desc: 'Demo 2\'s step: the desk survives and <code>err</code> is bound to the instance. The handler could log it and return a default. Instead it\'s going to raise something new, while this exception is still the "current" one.',
        lines: [4],
        memory: {
          frames: [
            { name: 'global', vars: [
      ...
      },
      {
        ...
        desc: 'A second instance appears. Two attributes on it now point back at the first: <code>__cause__</code>, set explicitly by <code>from err</code>, and <code>__context__</code>, which Python sets on <em>any</em> exception raised while another is being handled. <code>from</code> also sets <code>__suppress_context__</code> to <code>True</code>, which is how the traceback printer knows to say "direct cause" rather than "during handling".',
        lines: [5],
        memory: {
          frames: [
            { name: 'global', vars: [
          ...
          heap: [
            exception('runErr', ADDRS.runErr, 'RuntimeError', ADDRS.RuntimeErrorCls, 'bad input', {
              ...
              note: 'two links to the first exception, and cause is the one you set' }),
            exception('valErr', ADDRS.valErr, 'ValueError', ADDRS.ValueErrorCls6, "invalid literal for int() with base 10: 'x1'", {
      ...
      },
      {
        ...
        desc: 'Leaving the handler deletes <code>err</code> and clears the current-exception slot, as in demo 2. The <code>ValueError</code> would normally be freed here, but the <code>RuntimeError</code> holds it, so it travels along inside the second exception, traceback and all. <code>parse</code>\'s desk is cleared, and the module level has no handler.',
        lines: [5, 7],
        memory: {
          frames: [
            { name: 'global', badge: 'no handler', state: 'unwinding', vars: [
      ...
      },
      {
        ...
        desc: 'First the <code>ValueError</code> with its own traceback (<code>parse</code>, line 3), then the sentence <code>The above exception was the direct cause of the following exception:</code>, then the <code>RuntimeError</code> with its traceback (<code>&lt;module&gt;</code> line 7, <code>parse</code> line 5). Leave out <code>from err</code> and the link is still there through <code>__context__</code>, but the sentence becomes <code>During handling of the above exception, another exception occurred:</code>, with Python assuming the second error was an accident. <code>from None</code> hides the first one entirely.',
        lines: [7],
        memory: {
          frames: [
            { name: 'global', badge: 'exit status 1', vars: [
